...
const FilterBtn = (label: string, value: Filter, current: Filter) => {
	...
	return `<button type="button" data-filter="${value}" class="filter-btn text-xs px-3 py-1 rounded-full transition-all duration-150 cursor-pointer
...
export const createApp = (root: HTMLElement, initialState = createInitialState()) => {
	...
	const render = () => {
		...
		if (visible.length === 0) {
			listContainer.innerHTML = `<p class="text-zinc-600 text-xs text-center py-8">No tasks here</p>`;
		} else {
			listContainer.innerHTML = `<ul class="divide-y divide-zinc-800/60">${visible.map(TodoItem).join("")}</ul>`;
		}
